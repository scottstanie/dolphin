...
- `phase_linking.crlb_looks`: how the CRLB counts looks. `effective` (default) multiplies the SHP count by an effective-looks fraction measured once per stack from the intensity autocorrelation of sampled blocks (`phase_link.estimate_stack_effective_looks_fraction`), so one value applies to every block and ministack; `shp_count` treats every neighbor as independent; `sqrt_half_window` reproduces the previous output.
- `phase_linking.write_cumulative_closure_phase` (default True with `write_closure_phase`): the displacement workflow writes `cumulative_closure_phase_<date>.tif` next to the stitched interferograms, in meters when a wavelength is configured. `dolphin.closure.write_cumulative_closure_phase` replaces the body of `scripts/write-cumulative-closure-phase.py`; it orders triplets by their middle date and skips the long-pair triplets that start at a compressed SLC.
- The two-hop closure and split-half ratio rasters are cropped and stitched into `interferograms/` like the other per-ministack diagnostics, including for azimuth-block (NISAR frame) runs.
