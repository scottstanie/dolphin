...
from dolphin.io._core import DEFAULT_TIFF_OPTIONS_RIO, load_gdal
...
def unwrap_spurs(
    ...
    """Unwrap an interferogram using `spurs`.
    ...
    with ExitStack() as stack:
        if zero_where_masked and (mask_file is not None):
            ...
            igram = load_gdal(zeroed_ifg_file)
            corr = load_gdal(zeroed_corr_file)
        else:
            igram = load_gdal(ifg_filename)
            corr = load_gdal(corr_filename)
        ...
        logger.info("Unwrapping using spurs")
        unw = unwrap(np.angle(igram), backend="jax")  # , corr, nlooks)  # , mask=mask)

        logger.info("Writing unwrapped phase to raster file")

        igram_raster = stack.enter_context(snaphu.io.Raster(ifg_filename))
        ...
        # Spurs does not yet provide connected components. Instead, grow
