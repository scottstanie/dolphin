...
__all__ = ["BlockBounds", "crop_to_central", "split_frame_into_blocks"]
...
def split_frame_into_blocks(
    ...
    *,
    ...
    RuntimeError
        If the first input file has no usable EPSG.

    """
    halo = halo_rows if halo_rows is not None else _min_halo_rows(cfg)

    gdal_path = io.format_nc_filename(
        cfg.cslc_file_list[0], cfg.input_options.subdataset
    )
    try:
